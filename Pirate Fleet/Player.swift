...
class Player {
    ...
    var lastHitPenaltyCell: PenaltyCell? 
    ...
    func addPlayerShipsMinesMonsters(numberOfMines: Int = 0, numberOfSeamonsters: Int = 0) {
        ...
        for _ in 0..<numberOfMines {
            ...
            var mine = Mine(location: location, penaltyText: "NOPE", guaranteesHit: false)
            while !gridViewController.addMine(mine, playerType: .Computer) {
                location = RandomGridLocation()
                mine = Mine(location: location, penaltyText: "NOPE", guaranteesHit: false)
            }
        }
        ...
        for _ in 0..<numberOfSeamonsters {
            var location = RandomGridLocation()
            var seaMonster = SeaMonster(location: location, penaltyText: "HIT YA", guaranteesHit: true)
            while !gridViewController.addSeamonster(seaMonster, playerType: .Computer) {
                location = RandomGridLocation()
                seaMonster = SeaMonster(location: location, penaltyText: "HIT YA", guaranteesHit: true)
            }
        }
    }
}